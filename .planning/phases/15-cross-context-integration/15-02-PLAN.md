...
3. Optionally, manually test with a real markdown file in Finder:
   - Copy `comprehensive-v1.2.md` to Desktop
   - Build and install: `make install && make reload`
   - Select the file in Finder and press spacebar (popup)
   - Switch to column view and use preview pane (narrow)
   - Use fullscreen Quick Look
   - Toggle system dark mode and repeat
  </how-to-verify>
  <resume-signal>Type "approved" if snapshots look correct, or describe any rendering issues you see</resume-signal>
</task>
