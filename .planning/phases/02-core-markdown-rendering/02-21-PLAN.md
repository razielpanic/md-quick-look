...
The key insight: All runs of the same list item have the same ordinal. Only the first run (when ordinal changes) should get a prefix.
  ...
  <verify>
Run `xcodebuild build -scheme md-spotlighter -configuration Release -derivedDataPath build` and verify compilation succeeds with no errors.
...
Build verification:
```bash
xcodebuild build -scheme md-spotlighter -configuration Release -derivedDataPath build
```
